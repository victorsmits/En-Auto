  ...
  describe(Support.getTestDialectTeaser(flag), () => {
    ...
    it('creates a custom config folder', done => {
      gulp
        .src(Support.resolveSupportPath('tmp'))
        .pipe(helpers.clearDirectory())
        .pipe(helpers.runCli(flag + ' --config my-config/config/config.js'))
        .pipe(helpers.listFiles())
        ...
        gulp
          ...
          .pipe(helpers.teardown(() => {
            gulp
              ...
              .pipe(helpers.ensureContent('__dirname + \'/../config/config.js\''))
              .pipe(helpers.teardown(done));
          }));
      });
    ...
    it('does not overwrite an existing config.js file', done => {
      gulp
        .src(Support.resolveSupportPath('tmp'))
        .pipe(helpers.clearDirectory())
        .pipe(helpers.runCli(flag))
        .pipe(helpers.overwriteFile('foo', 'config/config.js'))
        .pipe(helpers.runCli(flag, { exitCode: 1 }))
        .pipe(helpers.readFile('config/config.js'))
